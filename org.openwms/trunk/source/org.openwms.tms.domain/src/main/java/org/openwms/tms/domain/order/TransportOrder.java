...
/**
 * A TransportOrder is used to move {@link TransportUnit}s from an current
...
public class TransportOrder extends AbstractEntity implements
	...
	/* ----------------------------- methods ------------------- */
	/**
	 * Create a new <code>TransportOrder</code>, set the creationDate and initial state.
	 */
	...
	/**
	 * Get the {@link TransportUnit} assigned to the
	...
	private void validateInitializationCondition() {
		if (transportUnit == null
				|| (targetLocation == null && targetLocationGroup == null)) {
			throw new IllegalStateException(
					"Not all properties set to turn TransportOrder into next state");
		}
	}
	...
	private void validateStateChange(TransportOrderState newState) {
		...
		if (getState().compareTo(newState) > 0) {
			...
			throw new IllegalStateException(
					"Turning back the state of a TransportOrder is not allowed");
		}
		if (getState() == TransportOrderState.CREATED) {
			if (newState != TransportOrderState.INITIALIZED
					&& newState != TransportOrderState.CANCELED) {
				// Don't allow to except the initialization
				throw new IllegalStateException(
						"The TransportOrder must first be initialized after creation");
			}
	 ...
	 *             <li>the <code>TransportOrder</code> is in state CREATED and shall be
	 *             manually turned into something else then INITIALIZED or
	 *             CANCELED</li>
	 *             <li>the <code>TransportOrder</code> is CREATED and shall be INITIALIZED
