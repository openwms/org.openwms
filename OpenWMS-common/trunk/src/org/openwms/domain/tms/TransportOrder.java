...
import org.openwms.domain.tms.values.Problem;

/**
 * 
 * A TransportOrder.
 * <p>
 * Is used to move <tt>TransportUnit</tt>s from an actual <tt>Location</tt>
 * to a target <tt>Location</tt>. A <tt>TransportOrder</tt> can only be
 * processed in a specific state (STARTED).
 * 
...
public class TransportOrder implements Serializable {
	...
	/**
	 * A priority level of the <tt>TransportOrder</tt>. The lower the value
	 * the lower the priority.<br>
	 * The priority level affects the execution of the <tt>TransportOrder</tt>.
	 * A high priored order will be processed faster than lower ones.
	 */
	...
	/**
	 * A <tt>LocationGroup</tt> can also set as target. When the
	 * <tt>TransportOrder</tt> will be started, a target must be set.
	 */
	@ManyToOne
	...
	/**
	 * Set the priority level of this <tt>TransportOrder</tt>.The lower the
	...
	/**
	 * Get the <tt>TransportUnit</tt> belonging to this
	 * <tt>TransportOrder</tt>.
	 * 
